  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-4 mb-8 p-6 bg-white rounded-lg border border-slate-200">
        {/* Eingabe */}
        <div
          onClick={() => setActiveBox('E')}
          className={`w-40 h-32 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all ${
            activeBox === 'E'
              ? 'bg-sky-100 border-2 border-sky-600 shadow-lg'
              : 'bg-white border-2 border-slate-300 hover:border-sky-400'
          }`}
        >
      ...
      <div className="bg-gradient-to-br from-sky-50 to-blue-50 p-6 rounded-lg border border-sky-200 min-h-40">
        {activeBox ? (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {evaData[activeBox].examples.map((example, i) => (
                  <span
                    key={i}
                    className="inline-block bg-sky-200 text-slate-800 text-xs px-3 py-1 rounded-full"
                  >
                    {example}
                  </span>
                ))}
          ...
          <div className="text-center text-slate-500 italic">
            Klicken Sie auf einen Block im Diagramm oben, um Details zu den
            Hardware-Komponenten anzuzeigen.
          </div>
        )}
      </div>
      ...
      <div className="mt-4 text-center text-sm text-slate-600">
        💡 Tip: Klicken Sie auf jeden Block, um mehr zu erfahren
      </div>
    </div>
  );
}
